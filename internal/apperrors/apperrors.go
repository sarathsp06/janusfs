...
// single translation point from any of them to a syscall.Errno.
// internal/mount is the only package permitted to
// call ToErrno; every other package returns one of these
...
// the source tree. Scoped to exactly the conditions those packages produce.
package apperrors
...
var (
	...
	// does not complete within its 10 s bound — errno EIO.
	...
	// --redact-buffer-max — errno EACCES (fail closed, same as a
	...
	// outside the source tree — errno ENOENT on follow, so the
...
// its assigned syscall.Errno. internal/mount is the only
// caller; every other package returns a sentinel and lets this
...
// went wrong, fail closed" condition, never leaked as some
