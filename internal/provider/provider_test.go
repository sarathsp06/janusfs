...
func TestMatchesPatternSig(t *testing.T) {
	p1 := &patterns.Pattern{Name: "jwt"}
	p2 := &patterns.Pattern{Name: "env-value"}
	p3 := &patterns.Pattern{Name: "aws-key"}

	sig0 := patternSignature(nil)
	sig1 := patternSignature([]*patterns.Pattern{p1})
	sig2 := patternSignature([]*patterns.Pattern{p1, p2})
	sig3 := patternSignature([]*patterns.Pattern{p1, p2, p3})

	if !matchesPatternSig(sig0, nil) {
		t.Error("expected matchesPatternSig true for nil")
	}
	if !matchesPatternSig(sig1, []*patterns.Pattern{p1}) {
		t.Error("expected matchesPatternSig true for p1")
	}
	if matchesPatternSig(sig1, []*patterns.Pattern{p2}) {
		t.Error("expected matchesPatternSig false for p2 vs sig1")
	}
	if !matchesPatternSig(sig2, []*patterns.Pattern{p2, p1}) { // order independent
		t.Error("expected matchesPatternSig true for p2, p1 (different order)")
	}
	if matchesPatternSig(sig2, []*patterns.Pattern{p1}) {
		t.Error("expected matchesPatternSig false for subset")
	}
	if !matchesPatternSig(sig3, []*patterns.Pattern{p3, p1, p2}) {
		t.Error("expected matchesPatternSig true for 3 patterns")
	}
}
